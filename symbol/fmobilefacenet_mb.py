...
def body(data, namespace=""):
    blocks = config.net_blocks
    conv_1 = Conv(data, num_filter=64, kernel=(3, 3), pad=(1, 1), stride=(2, 2), name=namespace+"conv_1")
    if blocks[0] == 1:
        conv_2_dw = Conv(conv_1, num_group=64, num_filter=64, kernel=(3, 3), pad=(1, 1), stride=(1, 1),
                         name=namespace+"conv_2_dw")
    else:
        conv_2_dw = Residual(conv_1, num_block=blocks[0], num_out=64, kernel=(3, 3), stride=(1, 1), pad=(1, 1),
                             num_group=64, name=namespace+"res_2")
    conv_23 = DResidual(conv_2_dw, num_out=64, kernel=(3, 3), stride=(2, 2), pad=(1, 1), num_group=128, name=namespace+"dconv_23")
    conv_3 = Residual(conv_23, num_block=blocks[1], num_out=64, kernel=(3, 3), stride=(1, 1), pad=(1, 1), num_group=128,
                      name=namespace+"res_3")
    conv_34 = DResidual(conv_3, num_out=128, kernel=(3, 3), stride=(2, 2), pad=(1, 1), num_group=256, name=namespace+"dconv_34")
    conv_4 = Residual(conv_34, num_block=blocks[2], num_out=128, kernel=(3, 3), stride=(1, 1), pad=(1, 1),
                      num_group=256, name=namespace+"res_4")
    conv_45 = DResidual(conv_4, num_out=128, kernel=(3, 3), stride=(2, 2), pad=(1, 1), num_group=512, name=namespace+"dconv_45")
    conv_5 = Residual(conv_45, num_block=blocks[3], num_out=128, kernel=(3, 3), stride=(1, 1), pad=(1, 1),
                      num_group=256, name=namespace+"res_5")
    conv_6_sep = Conv(conv_5, num_filter=512, kernel=(1, 1), pad=(0, 0), stride=(1, 1), name=namespace+"conv_6sep")
...
def get_symbol():
    ...
    conv_6_sep = body(data)
    conv6_dw1 = symbol_utils.Linear(conv_6_sep, num_filter=512, num_group=512, kernel=(7, 7), pad=(0, 0),
                                   stride=(1, 1), name="conv_6dw7_7")

    data2 = mx.symbol.Crop(data, offset=(39, 24), h_w=(64, 64))
    conv_6_sep2 = body(data2, namespace="crop")
    conv6_dw2 = symbol_utils.Linear(conv_6_sep2, num_filter=512, num_group=512, kernel=(4,4), pad=(0, 0), stride=(1, 1), name="cropconv_6dw7_7")

    conv6_dw = mx.symbol.Concat(conv6_dw1, conv6_dw2, dim=1, name="conv6_dw_concat")
    conv_6_f = mx.sym.FullyConnected(data=conv6_dw, num_hidden=num_classes, name='pre_fc1')
    fc1 = mx.sym.BatchNorm(data=conv_6_f, fix_gamma=True, eps=2e-5, momentum=config.bn_mom, name='fc1')
    ...
    digraph.render(filename='fmobilefacenet_mb')
